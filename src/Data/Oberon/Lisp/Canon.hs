...
module Data.Oberon.Lisp.Canon where
...
desugar = \case
  ...
  (S.Form fPos [S.Attr aPos "context", valExpr] : rest) ->
    let directive     = S.Form aPos [S.Symbol aPos "base", valExpr]
  ...
  (S.Attr pos "context" : valExpr : rest) ->
    let directive     = S.Form pos [S.Symbol pos "base", valExpr]
  ...
  (S.Form pos items : xs) ->
    S.Form pos (desugar items) : desugar xs
